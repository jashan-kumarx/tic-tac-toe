...
MongoDB — the engine is picked from the `DATABASE_URL` scheme (`mongodb://` → mongodb driver, anything
...
1. **Provision the DB** — DBs panel → “+” → *Provision new (Looper-managed)* →
   pick SQLite or MongoDB → name it e.g. `ttt-scores`.
...
- The DBs panel connection now lists a `scores` collection (table on
  SQLite) with the documents.
