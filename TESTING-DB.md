...
dev-provisioned databases feature end-to-end. One server covers all three
engines Looper can provision — the engine is picked from the `DATABASE_URL`
scheme (`postgresql://` → pg, `mongodb://` → mongodb driver, anything else →
a SQLite file path), so switching engines is only a matter of re-pointing the
env var at a different `{{db.<name>.url}}`.

## Steps (in Looper)

1. **Provision the DB** — DBs panel → “+” → *Provision new (Looper-managed)* →
   pick SQLite, PostgreSQL or MongoDB → name it e.g. `ttt-scores`.
...
- The DBs panel connection now lists a `scores` table (collection on Mongo)
  with the rows.
